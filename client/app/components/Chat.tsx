		...
		<>
			<section className="flex flex-col gap-3 px-8 w-full mx-auto overflow-y-scroll h-full">
				{messages?.map((m) => {
					...
					return (
						<div
							className={`w-fit p-2 border rounded-md border-white flex flex-col gap-2 ${user.id === m.sender_id ? 'items-end ml-auto' : 'items-start justify-start'}`}
							key={m.message_id}
						>
							<p className={`text-gray-900 text-sm`}>{u}</p>
